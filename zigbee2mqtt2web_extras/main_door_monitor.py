...
def _validate_cfg(cfg):
    cfg['contact_sensor_name']
    cfg['on_contact_action_name']
    cfg['lat_lon']

    if (cfg['sonos_name'] is None) != (cfg['chime_url'] is None):
        raise KeyError('MainDoorMonitor bad config: sonos_name and chime_url must be both null or both non-null')

    if 'skip_chime_timeout_secs' not in cfg:
        cfg['skip_chime_timeout_secs'] = 300

    if 'leaving_routine_timeout_secs' not in cfg:
        cfg['leaving_routine_timeout_secs'] = 60
    return cfg


class MainDoorMonitor:
    """ When a contact sensor opens, will play a chime on Sonos and activate a set of lights for a period of time """
    def __init__(self, zmw, cfg):
        ...
        thing.actions[self.cfg['on_contact_action_name']].value.on_change_from_mqtt = self._on_state_report

    def _on_state_report(self, has_contact):
        if has_contact:
            logger.info('%s: main door closed', self.cfg['contact_sensor_name'])
        else:
            if not light_outside(self.cfg['lat_lon']):
                logger.info('%s: main door open and is dark out, trigger leaving routine', self.cfg['contact_sensor_name'])
                self.trigger_leaving_routine()
            else:
                logger.info('%s: main door open', self.cfg['contact_sensor_name'])
    ...
    def _door_open_announce_and_block(self):
        if self._play_door_open_chime is True:
            ...
            self._zmw.registry.get_thing(self.cfg['sonos_name']).play_announcement(self.cfg['chime_url'], timeout_secs=20)
        else:
            logger.info("Door open, but we're skipping announcing...")

    def skip_next_door_open_chime(self):
        logger.info(f'Door chime will skip on door-open event for {self.cfg["skip_chime_timeout_secs"]} seconds')
        ...
        if self._skip_chime_bg is not None:
            logger.info('(We were already skipping chimes, adding more timeout)')
            self._skip_chime_bg.remove()

        self._skip_chime_bg = self._scheduler.add_job(
            func=restore_door_open_chime,
            next_run_time=datetime.datetime.now() + datetime.timedelta(seconds=self.cfg['skip_chime_timeout_secs']))

    def trigger_leaving_routine(self):
        logger.info('Starting leaving routine')

        if self._leaving_routine_bg is not None:
            logger.info('(Leaving routine already active: adding more timeout)')
            self._leaving_routine_bg.remove()
        else:
            # Only rebuild the active_managed_lamps list when a new leaving routine is triggered, to avoid inconsistencies
            for thing_name,pct in self.cfg['managed_lamps']:
                thing = self._zmw.registry.get_thing(thing_name)
                if thing.is_light_on():
                    logger.info("Light %s is already on, won't use it for leaving routine", thing_name)
                else:
                    logger.info("Light %s is managed by the leaving routine", thing_name)
        ...
        self._leaving_routine_bg = self._scheduler.add_job(
            ...
            next_run_time=datetime.datetime.now() + datetime.timedelta(seconds=self.cfg['leaving_routine_timeout_secs']))
